        ...
        for trace in log:  # в оригинале был прогресс от лога отдельной переменной, а лог глобальной, в процедуре
            ...
            if trace:  # if (!trace.isEmpty()) {
                if trace not in self.__visited_traces:
                    ...
                    flag = True
                    while flag:
                        # Find the elementary cycle in trace if there are one
                        ecyc = self.e_cyc(trace)
                        if self.__no_nested_cyc:
                            ecyc.sort()
                            self.add_invariant(ecyc)
                            flag = False
                        else:
                            if ecyc not in self.visited_cycs:
                            ...
                            #If this elementary cyc is already processed dont do it again
